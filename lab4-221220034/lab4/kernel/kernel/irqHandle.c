...
void syscallReadStdIn(struct StackFrame *sf) {
	...
	if(dev[STD_IN].value >= 0) {
		...
		char ch = 0;
		while(bufferHead != bufferTail && i < s) {
...
void syscallSemInit(struct StackFrame *sf) {
	...
	int offset = -1;
	for(offset = 0; offset < MAX_SEM_NUM; offset++) {
		if(sem[offset].state == 0) {
			sem[offset].value = (int32_t)sf->edx;
			sem[offset].state = 1;
			sem[offset].pcb.next = &(sem[offset].pcb);
			sem[offset].pcb.prev = &(sem[offset].pcb);
			break;		
		}
	}
...
void syscallSemWait(struct StackFrame *sf) {
	...
	if(sem[offset].state == 0 || offset < 0 || offset >= MAX_SEM_NUM) {
		pcb[current].regs.eax = -1;
		return;
	}
	pcb[current].regs.eax = 0;
	sem[offset].value -= 1;
	if(sem[offset].value < 0) {
		pcb[current].state = STATE_BLOCKED;
		pcb[current].sleepTime = -1;
		pcb[current].blocked.next = sem[offset].pcb.next;
		pcb[current].blocked.prev = &(sem[offset].pcb);
		sem[offset].pcb.next = &(pcb[current].blocked);
		(pcb[current].blocked.next)->prev = &(pcb[current].blocked);
		asm volatile("int $0x20");
	}
	return;
